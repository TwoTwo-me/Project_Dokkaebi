# Rejected Replay Fixture: Missing Worker Route Result Metadata

This replay fixture is rejected because Hammer route and result metadata are
incomplete.

## Replay identity

- **Replay ID:** `replay-37-rejected-worker-metadata`
...
- **Closeout evidence:** Worker route metadata and result packet link are
  missing.
- **Residual risk:** cannot prove which Hammer route produced the result.
- **Next state:** Blocked

## Replay decision

- **Expected replay result:** reject
- **Reason:** Worker route and result metadata are missing.
