...
if command -v gh >/dev/null 2>&1 && gh auth status -h github.com >/tmp/dokkaebi-manager-gh-auth.out 2>&1; then
  ...
  if has_write_project_scope "$scopes_line"; then
    ...
    if python3 "$ROOT/scripts/dokkaebi-project-status-sync.py" \
      --direction bidirectional \
      --apply \
      --record-state \
      --json >/tmp/dokkaebi-project-status-sync.out 2>/tmp/dokkaebi-project-status-sync.err; then
      status OK "GitHub Project Status auto-sync is clean"
    else
      status BLOCKED "GitHub Project Status auto-sync blocked; inspect /tmp/dokkaebi-project-status-sync.out and resolve drift/conflict"
    fi
  else
